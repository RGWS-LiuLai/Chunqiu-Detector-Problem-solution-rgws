...
>
> In plain terms, it's a mount leak.
>
...
> Try replacing meta-modules to resolve.

## TEE environment untrusted
> [From Tencent](https://github.com/Tencent/soter)
> Similar to WeChat's fingerprint.
> Waiting for module updates or

> Use HMA-OSS to hide the Soter system service application from the detector, and try to solve it (Fixed in 3.2fix? Not yet confirmed).
...
## Found property
Execute [this sh](https://github.com/mingzun09/Chunqiu-Detector-Problem-solution/blob/main/File/Found%20property.sh) to try and resolve.

## Tricky store Hook/2
> Replace with module [TEESimulator](https://github.com/JingMatrix/TEESimulator)
...
> Attempt 2
Delete the `/data/adb/Tricky store/security_patch.txt` file.
...
> To hide modified properties, add the [shamiko_Plus.sh](https://github.com/mingzun09/Chunqiu-Detector-Problem-solution/blob/main/File/shamiko_Plus.sh) file from the Shamiko module to the `/data/adb/service.d/` directory and restart to try and resolve.
...
>
> Or KSU processes and other factors are detected.
...
>
> Caused by device model modification modules? Troubleshoot yourself.
...
> Restart after flashing, then open the module's webUI to configure.
...
## AOSP Key
> Replace the `keybox.xml` file in the `/data/adb/Tricky Store/` directory.
> Alternatively, you can flash the [TS-Plugin](https://github.com/KOWX712/Tricky-Addon-Update-Target-List/releases/tag/v5.0-beta.1).
> After rebooting, open the module's webUI to configure the key.

## Boot Hash Mismatch
> Boot image hash mismatch.
>
> Usually, the hash becomes 0000 after unlocking the BL. Use [Native detector](https://t.me/rootdetector/49) to get the correct hash, then use the Tricky Store/[TEESimulator-RS](https://github.com/Enginex0/TEESimulator-RS) module and configure the hash using [TS-Plugin](https://github.com/KOWX712/Tricky-Addon-Update-Target-List/releases/tag/v5.0-beta.1) to resolve.
...
> It is also recommended to use [TS-Plugin](https://github.com/KOWX712/Tricky-Addon-Update-Target-List/releases/tag/v5.0-beta.1) for visual configuration of package names.
...
## Key Tampering (b)
> Issues caused by Tricky Store OSS （？）
>
> Try replacing it with the [TEESimulator-RS module](https://github.com/Enginex0/TEESimulator-RS).
...
>
> False positives may occur on international devices like Poco/Samsung (to be fixed).
